...
class LuzonCity extends StatefulWidget {
  ...
  const LuzonCity({super.key, required this.category, required this.level});
...
class _LuzonCityState extends State<LuzonCity> {
  ...
  void _checkAnswer(String selectedAnswer) {
    if (!_gameOver) {
      ...
      if (isCorrect) {
        ...
        audioCache.play('mohad.mp3');
      }
      bool isWrong = selectedAnswer != correctAnswer;
      if (isWrong) {
        AudioCache audioCache0 = AudioCache();
        audioCache0.play('lose.mp3');
      }
      showDialog(
        context: context,
        builder: (BuildContext context) {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Luzon Cities (Hard)'),
          leading: IconButton(
